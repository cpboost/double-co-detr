...
class LoadPairedImageFromFile(LoadImageFromFile):
    ...
    def __init__(self,
        ...
        self.file_client_args = file_client_args.copy()
        self.file_client = None
        self.rgb_folder_name = rgb_folder_name.replace('/', '')
    ...
    def __call__(self, results):
        ...
        img_bytes = self.file_client.get(filename)
        img_bytes_lwir = self.file_client.get(filename.replace(f'/{self.rgb_folder_name}/', '/tir/'))
        img = mmcv.imfrombytes(
            img_bytes, flag=self.color_type, channel_order=self.channel_order)
        img_lwir = mmcv.imfrombytes(
            img_bytes_lwir, flag=self.color_type, channel_order=self.channel_order)  
        if self.to_float32:
            img = img.astype(np.float32)
