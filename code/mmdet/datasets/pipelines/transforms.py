...
class RandomFlip:
    ...
    def __init__(self, flip_ratio=None, direction='horizontal', test_flip=False):   # clw modify
...
class PairedImagesRandomFlip(RandomFlip):
    ...
    def __call__(self, results):
        ...
        if self.test_flip==True:
            im = results['img']
            im = im[:, ::-1, :]
            results['img'] = im

            im_lwir = results['img_lwir']
            im_lwir = im_lwir[:, ::-1, :]
            results['img_lwir'] = im_lwir
        return results
...
@PIPELINES.register_module()
class PairedImagesNormalize(Normalize):
...
@PIPELINES.register_module()
class PairedImagesRandomCrop(RandomCrop):
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in ['img_lwir']:
            ...
            margin_w = max(img.shape[1] - crop_size[1], 0)
            # offset_h = np.random.randint(0, margin_h + 1)
            # offset_w = np.random.randint(0, margin_w + 1)   # clw modify：防止两次随机性造成的切图差异
            crop_y1, crop_y2 = offset_h, offset_h + crop_size[0]
            crop_x1, crop_x2 = offset_w, offset_w + crop_size[1]
...
class MixUp:
    ...
    def __call__(self, results):
        ...
        if random.random() > self.prob:
            return results     # clw modify TODO
...
class YOLOXHSVRandomAug:
    ...
    def __init__(self, hue_delta=5, saturation_delta=30, value_delta=30, prob=1.0):
        self.hue_delta = hue_delta
    ...
    def __call__(self, results):
        if random.random() > self.prob:      # clw modify TODO
            return results  
...
class CopyPaste:
    ...
    def _select_object(self, results):
        ...
        # print('============= max_num_pasted and num_pasted', max_num_pasted, num_pasted)
...
@PIPELINES.register_module()
class AutoAugmentCustom(object):
    def __init__(self, autoaug_type="v1"):
        """
        Args:
            autoaug_type (str): autoaug type, support v0, v1, v2, v3, test
        """
        super(AutoAugmentCustom, self).__init__()
...
@PIPELINES.register_module()
class PairedImagesAutoAugmentCustom(AutoAugmentCustom):
    ...
    def __call__(self, results):
        ...
        # from mmdet.datasets.pipelines.autoaugment_utils import distort_image_with_autoaugment
        from mmdet.datasets.pipelines.autoaugment_utils_pair import distort_image_with_autoaugment
        im, im_lwir, norm_gt_bbox = distort_image_with_autoaugment(im, im_lwir, norm_gt_bbox,
                                                          self.autoaug_type)
        gt_bbox[:, 0] = norm_gt_bbox[:, 1] * float(width)
        gt_bbox[:, 1] = norm_gt_bbox[:, 0] * float(height)
        gt_bbox[:, 2] = norm_gt_bbox[:, 3] * float(width)
...
class Stitch:
    ...
    def __init__(self,
        ...
        self.prob = prob
        self.extra_img_nums = 24

    def __call__(self, results):
        # if random.uniform(0, 1) > self.prob:
        #     return results
        results = self._stitch_transform(results)
        return results

    def get_indexes(self, dataset):
        # indexes = [random.randint(0, len(dataset)) for _ in range(3)]
        indexes = [random.randint(0, len(dataset)) for _ in range(self.extra_img_nums)]    # clw note: 25张stitch，相当于增加5倍缩小的小目标
        return indexes

    def _stitch_transform(self, results):
        """Mosaic transform function.
        ...
        stitch_img = np.full((int(self.img_scale[1] * int(math.sqrt(self.extra_img_nums+1))), int(self.img_scale[0] * int(math.sqrt(self.extra_img_nums+1)) ), 3), 0, dtype=results['img'].dtype)

    
        for i in range(self.extra_img_nums+1):
            if i == 0:
                results_patch = copy.deepcopy(results)
            else:
                results_patch = copy.deepcopy(results['mix_results'][i - 1])
            ...
            img_i = results_patch['img']
            h_i, w_i = img_i.shape[:2]
            assert h_i == self.img_scale[1] and w_i == self.img_scale[0]
            ...
            stitch_bboxes.append(gt_bboxes_i)
            stitch_labels.append(gt_labels_i)

        # import pdb; pdb.set_trace()
        if len(stitch_bboxes) > 0:
            stitch_bboxes = np.concatenate(stitch_bboxes, 0)
            stitch_labels = np.concatenate(stitch_labels, 0)

        results['img'] = stitch_img
        results['img_shape'] = stitch_img.shape
        results['gt_bboxes'] = stitch_bboxes
        results['gt_labels'] = stitch_labels
        # import pdb; pdb.set_trace()
        # import cv2; cv2.imwrite(results['img_info']['filename'], results['img'])
        return results
