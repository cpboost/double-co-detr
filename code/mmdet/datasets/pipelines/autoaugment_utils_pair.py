...
def cutout(image, image_lwir, pad_size, replace=0):
    ...
    image_lwir = np.where(
        np.equal(mask, 0),
        np.ones_like(
            image_lwir, dtype=image_lwir.dtype) * replace,
        image_lwir)
    return image.astype(np.uint8), image_lwir.astype(np.uint8)


def solarize(image, image_lwir, threshold=128):
    # For each pixel in the image, select the pixel
    # if the value is less than the threshold.
    # Otherwise, subtract 255 from the pixel.
    return np.where(image < threshold, image, 255 - image), np.where(image_lwir < threshold, image_lwir, 255 - image_lwir)


def solarize_add(image, image_lwir, addition=0, threshold=128):
    ...
    added_image_lwir = image_lwir.astype(np.int64) + addition
    added_image_lwir = np.clip(added_image_lwir, a_min=0, a_max=255).astype(np.uint8)
    return np.where(image < threshold, added_image, image), np.where(image_lwir < threshold, added_image_lwir, image_lwir)


def color(image, image_lwir, factor):
    """use cv2 to deal"""
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    degenerate = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)

    gray_lwir = cv2.cvtColor(image_lwir, cv2.COLOR_BGR2GRAY)
    degenerate_lwir = cv2.cvtColor(gray_lwir, cv2.COLOR_GRAY2BGR)
    return blend(degenerate, image, factor), blend(degenerate_lwir, image_lwir, factor)
...
def contrast(img, img_lwir, factor):
    img = ImageEnhance.Contrast(Image.fromarray(img)).enhance(factor)
    img_lwir = ImageEnhance.Contrast(Image.fromarray(img_lwir)).enhance(factor)
    return np.array(img), np.array(img_lwir)


def brightness(image, image_lwir, factor):
    """Equivalent of PIL Brightness."""
    degenerate = np.zeros_like(image)
    degenerate_lwir = np.zeros_like(image_lwir)
    return blend(degenerate, image, factor), blend(degenerate_lwir, image_lwir, factor)


# def posterize(image, image_lwir, bits):
#     """Equivalent of PIL Posterize."""
#     shift = 8 - bits
#     return np.left_shift(np.right_shift(image, shift), shift), np.left_shift(np.right_shift(image_lwir, shift), shift)


def rotate(image, image_lwir, degrees, replace):
    ...
    image_lwir = wrap(image_lwir)
    image_lwir = Image.fromarray(image_lwir)
    image_lwir = image_lwir.rotate(degrees)
    image_lwir = np.array(image_lwir, dtype=np.uint8)
    return unwrap(image, replace), unwrap(image_lwir, replace)
...
def _apply_bbox_augmentation(image, image_lwir, bbox, augmentation_func, *args):
    ...
    bbox_content_lwir = image_lwir[min_y:max_y + 1, min_x:max_x + 1, :]

    # Apply the augmentation function to the bbox portion of the image.
    augmented_bbox_content, augmented_bbox_content_lwir = augmentation_func(bbox_content, bbox_content_lwir, *args)      # clw note TODO
    ...
    augmented_bbox_content = np.pad(
        ...
        'constant',
        constant_values=1)
    augmented_bbox_content_lwir = np.pad(
        augmented_bbox_content_lwir, [[min_y, (image_height - 1) - max_y],
                                 [min_x, (image_width - 1) - max_x], [0, 0]],
        'constant',
        constant_values=1)
    ...
    image_lwir = image_lwir * mask_tensor + augmented_bbox_content_lwir
    return image.astype(np.uint8), image_lwir.astype(np.uint8)
...
def _apply_bbox_augmentation_wrapper(image, image_lwir, bbox, new_bboxes, prob,
    ...
    if func_changes_bbox:
        if should_apply_op:
            augmented_image, augmented_image_lwir, bbox = augmentation_func(image, image_lwir, bbox, *args)
        else:
            augmented_image, augmented_image_lwir, bbox = (image, image_lwir, bbox)
    else:
        if should_apply_op:
            augmented_image, augmented_image_lwir = _apply_bbox_augmentation(image, image_lwir, bbox,
                                                       augmentation_func, *args)
        else:
            augmented_image, augmented_image_lwir = image, image_lwir
    new_bboxes = _concat_bbox(bbox, new_bboxes)
    return augmented_image.astype(np.uint8), augmented_image_lwir.astype(np.uint8), new_bboxes


def _apply_multi_bbox_augmentation(image, image_lwir, bboxes, prob, aug_func,
    ...
    wrapped_aug_func = lambda _image, _image_lwir, bbox, _new_bboxes: _apply_bbox_augmentation_wrapper(_image, _image_lwir, bbox, _new_bboxes, prob, aug_func, func_changes_bbox, *args)
    ...
    body = lambda _idx, _images_and_bboxes: [
            _idx + 1, wrapped_aug_func(_images_and_bboxes[0],
                                        _images_and_bboxes[1],
                                         loop_bboxes[_idx],
                                         _images_and_bboxes[2])]
    while (cond(idx, (image, image_lwir, new_bboxes))):
        idx, (image, image_lwir, new_bboxes) = body(idx, (image, image_lwir, new_bboxes))
    ...
    if func_changes_bbox:
        ...
        final_bboxes = bboxes
    return image, image_lwir, final_bboxes


def _apply_multi_bbox_augmentation_wrapper(image, image_lwir, bboxes, prob, aug_func,
                                           func_changes_bbox, *args):
    """Checks to be sure num bboxes > 0 before calling inner function."""
    num_bboxes = len(bboxes)
    new_image = deepcopy(image)
    new_image_lwir = deepcopy(image_lwir)
    new_bboxes = deepcopy(bboxes)
    if num_bboxes != 0:
        new_image, new_image_lwir, new_bboxes = _apply_multi_bbox_augmentation(
            new_image, new_image_lwir, new_bboxes, prob, aug_func, func_changes_bbox, *args)
    return new_image, new_image_lwir, new_bboxes


def rotate_only_bboxes(image, image_lwir, bboxes, prob, degrees, replace):
    """Apply rotate to each bbox in the image with probability prob."""
    func_changes_bbox = False
    prob = _scale_bbox_only_op_probability(prob)
    return _apply_multi_bbox_augmentation_wrapper(
        image, image_lwir, bboxes, prob, rotate, func_changes_bbox, degrees, replace)


def shear_x_only_bboxes(image, image_lwir, bboxes, prob, level, replace):
    """Apply shear_x to each bbox in the image with probability prob."""
    func_changes_bbox = False
    prob = _scale_bbox_only_op_probability(prob)
    return _apply_multi_bbox_augmentation_wrapper(
        image, image_lwir, bboxes, prob, shear_x, func_changes_bbox, level, replace)


def shear_y_only_bboxes(image, image_lwir, bboxes, prob, level, replace):
    """Apply shear_y to each bbox in the image with probability prob."""
    func_changes_bbox = False
    prob = _scale_bbox_only_op_probability(prob)
    return _apply_multi_bbox_augmentation_wrapper(
        image, image_lwir, bboxes, prob, shear_y, func_changes_bbox, level, replace)


def translate_x_only_bboxes(image, image_lwir, bboxes, prob, pixels, replace):
    """Apply translate_x to each bbox in the image with probability prob."""
    func_changes_bbox = False
    prob = _scale_bbox_only_op_probability(prob)
    return _apply_multi_bbox_augmentation_wrapper(
        image, image_lwir, bboxes, prob, translate_x, func_changes_bbox, pixels, replace)


def translate_y_only_bboxes(image, image_lwir, bboxes, prob, pixels, replace):
    """Apply translate_y to each bbox in the image with probability prob."""
    func_changes_bbox = False
    prob = _scale_bbox_only_op_probability(prob)
    return _apply_multi_bbox_augmentation_wrapper(
        image, image_lwir, bboxes, prob, translate_y, func_changes_bbox, pixels, replace)


def flip_only_bboxes(image, image_lwir, bboxes, prob):
    """Apply flip_lr to each bbox in the image with probability prob."""
    func_changes_bbox = False
    prob = _scale_bbox_only_op_probability(prob)
    return _apply_multi_bbox_augmentation_wrapper(image, image_lwir, bboxes, prob,
                                                  np.fliplr, func_changes_bbox)


# def solarize_only_bboxes(image, image_lwir, bboxes, prob, threshold):
#     """Apply solarize to each bbox in the image with probability prob."""
#     func_changes_bbox = False
#     prob = _scale_bbox_only_op_probability(prob)
#     return _apply_multi_bbox_augmentation_wrapper(image, image_lwir, bboxes, prob, solarize,
#                                                   func_changes_bbox, threshold)


def equalize_only_bboxes(image, image_lwir, bboxes, prob):
    """Apply equalize to each bbox in the image with probability prob."""
    func_changes_bbox = False
    prob = _scale_bbox_only_op_probability(prob)
    return _apply_multi_bbox_augmentation_wrapper(image, image_lwir, bboxes, prob, equalize,
                                                  func_changes_bbox)


def cutout_only_bboxes(image, image_lwir, bboxes, prob, pad_size, replace):
    """Apply cutout to each bbox in the image with probability prob."""
    func_changes_bbox = False
    prob = _scale_bbox_only_op_probability(prob)
    return _apply_multi_bbox_augmentation_wrapper(
        image, image_lwir, bboxes, prob, cutout, func_changes_bbox, pad_size, replace)
...
def rotate_with_bboxes(image, images_lwir, bboxes, degrees, replace):
    # Rotate the image.
    image, images_lwir = rotate(image, images_lwir, degrees, replace)
    ...
    return image, images_lwir, new_bboxes


def translate_x(image, image_lwir, pixels, replace):
    """Equivalent of PIL Translate in X dimension."""
    image = Image.fromarray(wrap(image))
    image_lwir = Image.fromarray(wrap(image_lwir))
    image = image.transform(image.size, Image.AFFINE, (1, 0, pixels, 0, 1, 0))
    image_lwir = image_lwir.transform(image_lwir.size, Image.AFFINE, (1, 0, pixels, 0, 1, 0))
    return unwrap(np.array(image), replace), unwrap(np.array(image_lwir), replace)


def translate_y(image, image_lwir, pixels, replace):
    """Equivalent of PIL Translate in Y dimension."""
    image = Image.fromarray(wrap(image))
    image_lwir = Image.fromarray(wrap(image_lwir))
    image = image.transform(image.size, Image.AFFINE, (1, 0, 0, 0, 1, pixels))
    image_lwir = image_lwir.transform(image_lwir.size, Image.AFFINE, (1, 0, 0, 0, 1, pixels))
    return unwrap(np.array(image), replace), unwrap(np.array(image_lwir), replace)
...
def translate_bbox(image, image_lwir, bboxes, pixels, replace, shift_horizontal):
    ...
    """
    if shift_horizontal:
        image, image_lwir = translate_x(image, image_lwir, pixels, replace)
    else:
        image, image_lwir = translate_y(image, image_lwir, pixels, replace)
    ...
    return image.astype(np.uint8), image_lwir.astype(np.uint8), new_bboxes


def shear_x(image, image_lwir, level, replace):
    ...
    image_lwir = Image.fromarray(wrap(image_lwir))
    image_lwir = image_lwir.transform(image_lwir.size, Image.AFFINE, (1, level, 0, 0, 1, 0))
    return unwrap(np.array(image), replace), unwrap(np.array(image_lwir), replace)


def shear_y(image, image_lwir, level, replace):
    ...
    image_lwir = Image.fromarray(wrap(image_lwir))
    image_lwir = image_lwir.transform(image_lwir.size, Image.AFFINE, (1, 0, 0, level, 1, 0))
    return unwrap(np.array(image), replace), unwrap(np.array(image_lwir), replace)
...
def shear_with_bboxes(image, image_lwir, bboxes, level, replace, shear_horizontal):
    ...
    """
    if shear_horizontal:
        image, image_lwir = shear_x(image, image_lwir, level, replace)
    else:
        image, image_lwir = shear_y(image, image_lwir, level, replace)
...
def autocontrast(image, image_lwir):
    ...
    s1 = scale_channel(image_lwir[:, :, 0])
    s2 = scale_channel(image_lwir[:, :, 1])
    s3 = scale_channel(image_lwir[:, :, 2])
    image_lwir = np.stack([s1, s2, s3], 2)
    return image, image_lwir


def sharpness(image, image_lwir, factor):
    """Implements Sharpness function from PIL."""
    orig_image = image
    orig_image_lwir = image_lwir
    image = image.astype(np.float32)
    image_lwir = image_lwir.astype(np.float32)
    # Make image 4D for conv operation.
    # SMOOTH PIL Kernel.
    kernel = np.array([[1, 1, 1], [1, 5, 1], [1, 1, 1]], dtype=np.float32) / 13.
    result = cv2.filter2D(image, -1, kernel).astype(np.uint8)
    result_lwir = cv2.filter2D(image_lwir, -1, kernel).astype(np.uint8)

    # Blend the final result.
    return blend(result, orig_image, factor), blend(result_lwir, orig_image_lwir, factor)


def equalize(image, image_lwir):
...
def _cutout_inside_bbox(image, image_lwir, bbox, pad_fraction):
    ...
    mean_lwir = np.mean(image_lwir[min_y:max_y + 1, min_x:max_x + 1], axis=(0, 1))
    ...
    mask = np.tile(mask, [1, 1, 3])
    return mask, mean, mean_lwir


def bbox_cutout(image, image_lwir, bboxes, pad_fraction, replace_with_mean):
    ...
    def apply_bbox_cutout(image, image_lwir, bboxes, pad_fraction):
        ...
        mask, mean, mean_lwir = _cutout_inside_bbox(image, image_lwir, chosen_bbox, pad_fraction)
        ...
        replace_lwir = mean_lwir if replace_with_mean else [128] * 3
        ...
        image_lwir = np.where(
            np.equal(mask, 0),
            np.ones_like(
                image_lwir, dtype=image_lwir.dtype) * replace_lwir,
            image_lwir).astype(image_lwir.dtype)
        return image, image_lwir

    # Check to see if there are boxes, if so then apply boxcutout.
    if len(bboxes) != 0:
        image = apply_bbox_cutout(image, image_lwir, bboxes, pad_fraction)
...
NAME_TO_FUNC = {
        ...
        # pylint:disable=g-long-lambda
        'TranslateX_BBox': lambda image, image_lwir, bboxes, pixels, replace: translate_bbox(
                image, image_lwir, bboxes, pixels, replace, shift_horizontal=True),
        'TranslateY_BBox': lambda image, image_lwir, bboxes, pixels, replace: translate_bbox(
                image, image_lwir, bboxes, pixels, replace, shift_horizontal=False),
        'ShearX_BBox': lambda image, image_lwir, bboxes, level, replace: shear_with_bboxes(
                image, image_lwir, bboxes, level, replace, shear_horizontal=True),
        'ShearY_BBox': lambda image, image_lwir, bboxes, level, replace: shear_with_bboxes(
                image, image_lwir, bboxes, level, replace, shear_horizontal=False),
        # pylint:enable=g-long-lambda
...
def bbox_wrapper(func):
    ...
    def wrapper(images, images_lwir, bboxes, *args, **kwargs):   # clw note TODO
        return (func(images, images_lwir, *args, **kwargs), bboxes)
...
def _apply_func_with_prob(func, image, image_lwir, args, prob, bboxes):
    """Apply `func` to image w/ `args` as input with probability `prob`."""
    assert isinstance(args, tuple)
    # assert 'bboxes' == inspect.getargspec(func)[0][1]
    assert 'bboxes' == inspect.getargspec(func)[0][2]    # clw modify TODO: 索引[0][0]是image, [0][1]是image_lwir, [0][2]是bboxes
    ...
    if should_apply_op:
        ...
        # augment_images_all = func(image, image_lwir, bboxes, *args)
        # print('======== augment_images_all:', augment_images_all)
        # augmented_image, augmented_image_lwir = augment_images_all
        ################### clw modify TODO
        tmp =  func(image, image_lwir, bboxes, *args)
        try:
            augmented_image, augmented_image_lwir, augmented_bboxes = tmp
        except:
            assert len(tmp) == 2
            augmented_image, augmented_image_lwir = tmp[0]
            augmented_bboxes = tmp[1]
        ######################
    else:
        augmented_image, augmented_image_lwir, augmented_bboxes = (image, image_lwir, bboxes)
    return augmented_image, augmented_image_lwir, augmented_bboxes


def select_and_apply_random_policy(policies, image, image_lwir, bboxes):
    ...
    for (i, policy) in enumerate(policies):
        if i == policy_to_select:
            image, image_lwir, bboxes = policy(image, image_lwir, bboxes)
    return (image, image_lwir, bboxes)


def build_and_apply_nas_policy(policies, image, image_lwir, bboxes, augmentation_hparams):
    ...
    for policy in policies:
        ...
        def make_final_policy(tf_policy_):
            def final_policy(image_, image_lwir_, bboxes_):
                for func, prob, args in tf_policy_:
                    image_, image_lwir_, bboxes_ = _apply_func_with_prob(func, image_, image_lwir_, args,
                                                            prob, bboxes_)
                return image_, image_lwir_, bboxes_

            return final_policy

        tf_policies.append(make_final_policy(tf_policy))

    augmented_images, augmented_images_lwir, augmented_bboxes = select_and_apply_random_policy(
        tf_policies, image, image_lwir, bboxes)
    # If no bounding boxes were specified, then just return the images.
    return (augmented_images, augmented_images_lwir, augmented_bboxes)


# TODO(barretzoph): Add in ArXiv link once paper is out.
def distort_image_with_autoaugment(image, image_lwir, bboxes, augmentation_name):
    ...
    return build_and_apply_nas_policy(policy, image, image_lwir, bboxes,
                                      augmentation_hparams)
