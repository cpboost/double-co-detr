...
class VisionRotaryEmbeddingFast(nn.Module):
    ...
    def forward(self, t): 
        ...
        # print('======== self.freqs_cos.shape:', self.freqs_cos.shape)
        # print('======== self.freqs_sin.shape:', self.freqs_sin.shape)
        # print('======== rotate_half(t).shape:', rotate_half(t).shape)
...
class ViT(BaseModule):
    ...
    def forward(self, x):
        ...
        # import pdb; pdb.set_trace()
        return outputs   # clw note: (bs, 1024, h//16, w//16)
...
class TwoStreamViT(BaseModule):
    ...
    def __init__(
        ...
        super(TwoStreamViT, self).__init__(init_cfg=init_cfg)
        self.pretrain_use_cls_token = pretrain_use_cls_token
        ...
        if use_abs_pos:
            ...
            num_positions = (num_patches + 1) if pretrain_use_cls_token else num_patches
            # self.pos_embed = nn.Parameter(torch.zeros(1, num_positions, embed_dim))
            self.pos_embed_vis = nn.Parameter(torch.zeros(1, num_positions, embed_dim))
            self.pos_embed_lwir = nn.Parameter(torch.zeros(1, num_positions, embed_dim))
        else:
            # self.pos_embed = None
            self.pos_embed_vis = None
            self.pos_embed_lwir = None
        ...
        #     ft_seq_len=hw_seq_len if intp_freq else None,
        # )
        self.rope_win_vis = VisionRotaryEmbeddingFast(
            dim=half_head_dim,
            pt_seq_len=pt_hw_seq_len,
            ft_seq_len=window_size if intp_freq else None,
        )
        self.rope_glb_vis = VisionRotaryEmbeddingFast(
            dim=half_head_dim,
            pt_seq_len=pt_hw_seq_len,
            ft_seq_len=hw_seq_len if intp_freq else None,
        )
        self.rope_win_lwir = VisionRotaryEmbeddingFast(
            dim=half_head_dim,
            pt_seq_len=pt_hw_seq_len,
            ft_seq_len=window_size if intp_freq else None,
        )
        self.rope_glb_lwir = VisionRotaryEmbeddingFast(
            dim=half_head_dim,
            pt_seq_len=pt_hw_seq_len,
            ft_seq_len=hw_seq_len if intp_freq else None,
        )
        if not use_lsj:
            # self.rope_glb = None
            self.rope_glb_vis = None
            self.rope_glb_lwir = None

        # stochastic depth decay rule
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]

        # self.blocks = nn.ModuleList()
        self.blocks_vis = nn.ModuleList()
        self.blocks_lwir = nn.ModuleList()
        for i in range(depth):
            # block = Block(
            #     dim=embed_dim,
            #     num_heads=num_heads,
            ...
            block_vis = Block(
                ...
                window_size=window_size if i in window_block_indexes else 0,
                use_residual_block=i in residual_block_indexes,
                rope=self.rope_win_vis if i in window_block_indexes else self.rope_glb_vis,
                xattn=xattn
            )
            block_lwir = Block(
                dim=embed_dim,
                ...
                window_size=window_size if i in window_block_indexes else 0,
                use_residual_block=i in residual_block_indexes,
                rope=self.rope_win_lwir if i in window_block_indexes else self.rope_glb_lwir,
                xattn=xattn
            )
            if use_act_checkpoint:
                # TODO: use torch.utils.checkpoint
                ...
                # block = checkpoint_wrapper(block)
                block_vis = checkpoint_wrapper(block_vis)
                block_lwir = checkpoint_wrapper(block_lwir)
            self.blocks_vis.append(block_vis)
            self.blocks_lwir.append(block_lwir)

        self._out_feature_channels = {out_feature: embed_dim}
        self._out_feature_strides = {out_feature: patch_size}
        self._out_features = [out_feature]
        self.out_norm = nn.LayerNorm(embed_dim)
        # self.out_norm_vis = nn.LayerNorm(embed_dim)
        # self.out_norm_lwir = nn.LayerNorm(embed_dim)

        # if self.pos_embed is not None:
        #     nn.init.trunc_normal_(self.pos_embed, std=0.02)
        if self.pos_embed_vis is not None:
            nn.init.trunc_normal_(self.pos_embed_vis, std=0.02)
        if self.pos_embed_lwir is not None:
            nn.init.trunc_normal_(self.pos_embed_lwir, std=0.02)
    ...
    def forward(self, x_vis, x_lwir):
        ...
        if self.pos_embed_vis is not None and self.pos_embed_lwir is not None:
            x_vis = x_vis + get_abs_pos(
                self.pos_embed_vis, self.pretrain_use_cls_token, (x_vis.shape[1], x_vis.shape[2])
            )
            x_lwir = x_lwir + get_abs_pos(
                self.pos_embed_lwir, self.pretrain_use_cls_token, (x_vis.shape[1], x_vis.shape[2])
            )

        for i, (blk_vis, blk_lwin) in enumerate(zip(self.blocks_vis, self.blocks_lwir)):
            x_vis = blk_vis(x_vis)
            print('x_vis.shape:', x_vis.shape)   # 全部是 (bs, h//16, w//16, 1024)
            # x_lwir = blk_lwin(x_lwir)
        import pdb; pdb.set_trace()

        

        outputs = [self.out_norm(x).permute(0, 3, 1, 2).contiguous()]
        # outputs = [self.out_norm_vis(x_vis).permute(0, 3, 1, 2).contiguous()]
        # outputs = [self.out_norm_lwir(x_lwir).permute(0, 3, 1, 2).contiguous()]
        # import pdb; pdb.set_trace()
        return outputs   # clw note: (bs, 1024, h//16, w//16)
