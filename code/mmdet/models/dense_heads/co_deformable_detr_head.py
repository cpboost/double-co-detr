...
class CoDeformDETRHead(DETRHead):
    ...
    def _get_bboxes_single(self,
        ...
        # score_thr = self.test_cfg.get('score_thr', 0.01)   # clw modify TODO
        score_thr = 0.00001
        if with_nms:
    ...
    def aug_test_bboxes(self, feats, img_metas, rescale=False):   # clw modify: 比如水平翻转，feats和img_metas都是含有两个元素的list;  feats[0][0].shape: (1, 256, h//4, w//4) ~ feats[0][5]: (1, 256, h//64, w//64)
                                                                  #             img_metas信息：[{'filename': '/cpfs/user/caoliwei/Dataset/2024GAIIC_track1/ir/val/images/00001.jpg', 
                                                                  #                             'ori_filename': '00001.jpg', 'ori_shape': (512, 640, 3), 'img_shape': (512, 640, 3), 
                                                                  #                            'pad_shape': (640, 640, 3), 'scale_factor': array([1., 1., 1., 1.], dtype=float32), 
                                                                  #  'flip': False,              # 第二个元素的就是 True 
                                                                  # 'flip_direction': None,        # 第二个元素的就是 'horizontal
        ...
        for x, img_meta in zip(feats, img_metas):
            ...
            # mask_score = bbox_outputs[0][:, 4] > 0.91
            # aug_bboxes.append(bbox_outputs[0][mask_score][:, :4])
            # aug_scores.append(bbox_outputs[0][mask_score][:, 4])
            # aug_labels.append(bbox_outputs[1][mask_score])
        ...
        #                                     # merged_labels, dict(type='soft_nms', iou_threshold=0.6))
        #                                     # merged_labels, dict(type='soft_nms', iou_threshold=0.7))
        #                                     merged_labels, dict(type='soft_nms', iou_threshold=0.8))
        # det_bboxes = det_bboxes   # [:self.test_cfg.max_per_img]   # 默认300个
        ...
        merged_bboxes, merged_scores = self.merge_aug_bboxes_for_wbf(
            aug_bboxes, aug_scores, img_metas)
        # import pdb; pdb.set_trace()
        merged_labels = aug_labels

        img_h_for_norm = 512.0
        img_w_for_norm = 640.0
        merged_bboxes_norm = []  # wbf 需要把box归一化到从0到1
        for merged_bbox in merged_bboxes:
            merged_bbox[:, 0::2] /= img_w_for_norm
            merged_bbox[:, 1::2] /= img_h_for_norm
            merged_bboxes_norm.append(merged_bbox)
        merged_bboxes = merged_bboxes_norm
        
        # det_bboxes, det_scores, det_labels = weighted_boxes_fusion(merged_bboxes, merged_scores, merged_labels, weights=None, iou_thr=0.8, skip_box_thr=0.0001, conf_type='avg')
        det_bboxes, det_scores, det_labels = weighted_boxes_fusion(merged_bboxes, merged_scores, merged_labels, weights=None, iou_thr=0.8, skip_box_thr=0.0001, conf_type='max')
        det_bboxes[:, 0::2] *= img_w_for_norm
        det_bboxes[:, 1::2] *= img_h_for_norm
        # import pdb; pdb.set_trace()
        det_bboxes = np.concatenate((det_bboxes, det_scores[:, np.newaxis]), axis=1)
        # # import pdb; pdb.set_trace()
        #######################################
    ...
    def simple_test_bboxes(self, feats, img_metas, rescale=False):
        ...
        # import pdb; pdb.set_trace()
        # results_list = self.get_bboxes(*outs, img_metas, rescale=rescale, with_nms=False)   # clw modify TODO: 基本和默认的  {'type': 'soft_nms', 'iou_threshold': 0.8} 指标一样
        return results_list