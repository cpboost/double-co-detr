...
def inference_detector_kaist(model, img):
    cfg = model.cfg
    device = next(model.parameters()).device  # model device

    # cfg.data.test.pipeline = replace_ImageToTensor(cfg.data.test.pipeline)   # clw delete TODO
    # print('===== cfg.data.test.pipeline before:', cfg.data.test.pipeline)
    ...
    # print('===== cfg.data.test.pipeline after modify:', cfg.data.test.pipeline)
...
#     # prepare data
#     data = dict(img_path=imgs, img_lwir_path=imgs_lwir, img_id=0)
#     data = test_pipeline(data)
